...
#include "examples/peerconnection/client/nativehandlebuffer.h"
#include "examples/peerconnection/client/xwindownativeframe.h"
...
gboolean HandleUIThreadCallback(gpointer data) {
  ...
  std::cout << "main_wnd.cc 127 lines HandleUIThreadCallback" << std::endl;
...
GtkMainWnd::GtkMainWnd(const char* server,
  ...
  port_ = buffer;
  std::cout << "main_wnd.cc 167 lines  port=" << port << std::endl;
}
...
void GtkMainWnd::QueueUIThreadCallback(int msg_id, void* data) {
  std::cout << "main_wnd.cc 223 lines QueueUIThreadCallback" << std::endl;
...
void GtkMainWnd::SwitchToConnectUI() {
  ...
  std::cout << "main_wnd.cc 256 lines SwitchToConnectUI()" << std::endl;
...
void GtkMainWnd::OnClicked(GtkWidget* widget) {
  ...
  callback_->StartLogin(server_, port);      //登录服务器
}
...
void GtkMainWnd::OnRowActivated(GtkTreeView* tree_view,
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
    ...
    if (id != -1)
        std::cout << "main_wnd id=" << id << std::endl;   // 第一次主动连接，显示id=1
...
GtkMainWnd::VideoRenderer::VideoRenderer(
    ...
    renderer_.SetLocalARGBFile("/home/jack-ma/webrtc_m76/src/LocalARGBFile.txt");
    renderer_.SetLocalLatencyFile("/home/jack-ma/webrtc_m76/src/LocalLatencyFile.txt");
...
void GtkMainWnd::VideoRenderer::OnFrame(const webrtc::VideoFrame& video_frame) {
  ...
  if(!renderer_.GetLocalRenderFILE()) {
    std::cout << "main_wnd.cc 569 lines GetLocalRenderFILE failed!" << std::endl;
  }
  
  fprintf(renderer_.GetLocalRenderFILE(), ",%ld", (video_frame.timestamp_us()-m_RecordTime));
  fflush(renderer_.GetLocalRenderFILE());
  m_RecordTime = video_frame.timestamp_us();
  ...
  std::cout << "main_wnd.cc 554 lines Onframe" << std::endl;
  gdk_threads_enter();  //  线程中对程序界面进行动态修改
  ...
  std::cout << "buffer->width()=" << buffer->width() << " buffer->height()" 
