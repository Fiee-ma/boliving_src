...
CFileFrameGenerator::CFileFrameGenerator(int width, int height, int fps, std::string filename)
{
  LOG_DEBUG("");
  m_countTag = 1;
  m_width = width;
  ...
  m_fd.open(m_videoPath.c_str(), std::ios::in);
  if (m_fd.is_open())
  {
...
owt::base::VideoFrameGeneratorInterface::VideoFrameCodec CFileFrameGenerator::GetType()
{
  LOG_DEBUG("");
  return m_type;
}
...
void CFileFrameGenerator::SetPublishTimeFile(const std::string &file)
{
  LOG_DEBUG("");
